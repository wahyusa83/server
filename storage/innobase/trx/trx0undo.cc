...
trx_undo_page_get_first_rec(const buf_block_t *block,
                                     ...
                                     bool shared, mtr_t *mtr)
{
  ...
  return block
    ? trx_undo_page_get_last_rec(block, page_no, block->page.frame() + offset)
    : nullptr;
}
...
@return undo log record, the page latched, NULL if none */
trx_undo_rec_t*
trx_undo_get_next_rec(const buf_block_t *&block, const byte *rec,
                      uint32_t page_no, uint16_t offset, mtr_t *mtr)
{
  ...
  return trx_undo_get_next_rec_from_next_page(block, page_no,
                                              page_align(rec) + offset,
                                              RW_S_LATCH, mtr);
}
                       ...
                       mtr_t *mtr, dberr_t *err)
{
  ...
  const byte *header= block->page.frame() + offset;

  if (trx_undo_rec_t *rec=
      trx_undo_page_get_first_rec(block, page_no, header))
    return rec;
...
inline void UndorecApplier::assign_rec(const byte *offset)
{
  this->offset= page_offset(offset);
  this->undo_rec= trx_undo_rec_copy(offset, heap);
}
...
ATTRIBUTE_COLD void trx_t::apply_log()
{
  ...
  for (;;)
  {
    ...
    while (rec)
    {
      ...
      rec= trx_undo_page_get_next_rec(block, page + log_applier.get_offset(),
                               ...
                               const XID &xid, mtr_t *mtr)
{
  ...
  const ulint xid_length= static_cast<ulint>(xid.gtrid_length
                                             + xid.bqual_length);
  mtr->memcpy(*block, log_hdr + TRX_UNDO_XA_XID, xid.data, xid_length);
