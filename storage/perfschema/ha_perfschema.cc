...
static int pfs_discover_table(handlerton *hton, THD *thd, TABLE_SHARE *share)
{
  ...
  if ((pfs_share= find_table_share(share->db, share->table_name)))
...
{
  return MY_TEST(find_table_share(Lex_cstring_strlen(db),
...
int ha_perfschema::open(const char *name, int mode, uint test_if_locked)
{
  ...
  m_table_share= find_table_share(table_share->db, table_share->table_name);
  if (! m_table_share)
...
int ha_perfschema::delete_table(const char *name)
{
  ...
  *ptr = '\0';
  ...
  share = find_table_share(Lex_cstring_strlen(db_name),
                           Lex_cstring_strlen(table_name));
  if (share != NULL) {
    if (share->m_optional) {
      /*
