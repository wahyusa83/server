...
class Write_record
{
  ...
  const uint last_unique_key;
  const bool use_triggers;
  const bool versioned;
  const bool has_delete_triggers;
  const bool referenced_by_fk;
  static uint get_last_unique_key(TABLE *table);
  // FINALIZATION
...
public:
  ...
  */
  Write_record(THD *thd, TABLE *table, COPY_INFO *info,
          ...
          last_unique_key(get_last_unique_key(table)),
