                               ...
                               engine_option_value **end)
{
  ...
  if (opt)
  {
    opt->value= Lex_ident_table_option_value(); /* remove previous value */
...
static bool set_one_value(ha_create_table_option *opt,
                          THD *thd, const Lex_ident_table_option_value *value,
                          void *base,
                          bool suppress_warning,
                          MEM_ROOT *root)
{
                       ...
                       bool suppress_warning, MEM_ROOT *root)
{
  ...
  for (opt= rules; rules && opt->name; opt++)
  {
    ...
    {
      Lex_ident_table_option_value default_val;
      ...
      */
          ...
          (thd->lex->sql_command == SQLCOM_CREATE_TABLE || seen))
      {
        ...
        if (!sysvar->session_is_default(thd))
        {
          ...
          {
            Lex_ident_table_option_name name(opt->name, opt->name_length);
...
#define need_buff(N)  if (buff + (N) >= buff_end) return NULL
  ...
  engine_option_value *ptr=
      new (root) engine_option_value(Lex_ident_table_option_name(name),
