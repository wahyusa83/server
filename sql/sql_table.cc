                                    ...
                                    const LEX_CSTRING table_name)
{
  ...
  */
  ...
  static const Lex_ident_column ignore_key= ""_Lex_ident_column;
  ...
  *key_count= 0;
  ...
  while ((key=key_iterator++))
  {
    ...
    if (key->name.str != ignore_key.str)
  ...
  while ((key=key_iterator++))
  {
    if (key->name.str == ignore_key.str || key->type == Key::FOREIGN_KEY)
      continue;
  ...
  for (; (key=key_iterator++) ; key_number++)
  {
    ...
    if (key->name.str == ignore_key.str)
    {
      ...
      do
	key=key_iterator++;
      while (key && key->name.str == ignore_key.str);
      if (!key)
	break;
    }
                      ...
                      uint *key_count, LEX_CUSTRING *frm)
{
  ...
  if (create_info->tmp_table())
  {
    ...
    */
    TABLE *tmp_table= thd->find_temporary_table(Lex_ident_db(db),
