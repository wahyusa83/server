...
{
  const Lex_ident_db db= Lex_cstring_strlen((const char*) element->m_key);
  ...
  {						// Using table locks
    ...
    for (table=thd->open_tables; table ; table=table->next)
    {
	  ...
	  !memcmp(table->s->table_cache_key.str, key, key_length))
      {
        ...
        if (!my_charset_utf8mb4_tolower_ci.strnncoll(
              ...
              bool any_privileges, uint *hidden_bit_fields, bool returning_field)
{
  ...
  */
  ...
  for (TABLE_LIST *tables= first; tables; tables= tables->*next)
  {
    ...
    if ((table_name && !Lex_ident_table(Lex_cstring_strlen(table_name)).
                          streq(tables->alias)) ||
